...
| 送信ボタン | フォームの送信ボタン | [ソースコード](src/components/button/SubmitButton.vue) | [Storybook](https://mysd33.github.io/sample-vue-tailwind/?path=/docs/components-button-submitbutton--docs) |
...
|-------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------|-------------------------|
| ページ生成                         | Vue.js（SPAフレームワーク）やTailwindCSS（CSSフレームワーク）の標準機能を用いて、画面レイアウトや、スタイルやテーマを実装支援する機能を提供する。                                                                                                                           | -    | -                       |
| データバインディング                    | 画面表示する項目と、画面項目、画面状態を保持する画面データを紐づけて、値の変更を検知し、同期する機能を提供する。<br/>Vue.jsのref、reactveおよびv-bind、v-modelで一方向または双方向バインドを実現する。<br/>また、コンポーネントの親子間でのデータのやり取りはProps、子供から親への通知はEmit、親子孫といった3階層以上でのやり取りにはProvide/Injectを用いる。 | -    | -                       |
| イベント処理制御                      | 初期画面表示やクリック等のイベントに対して、一連クライアント処理を起動する機能を提供する。<br/>Vue.jsのv-onでイベント（Emit）と紐づけ、formの送信に関するイベント処理制御は、VeeValidateを使用する。                                                                                            | -    | -                       |
| 二重送信防止                        | VeeValidateのisSubmittingの状態管理機能を用いて、ボタンクリック時にイベント処理終了までボタンを非活性化（disable）することで、ボタンのダブルクリック等により同じ処理が誤って複数回実行され誤動作することを防止する。                                                                                      | -    | -                       |
| 入力チェック                        | VeeValidateとyupにより、画面の入力時における単項目チェック、相関項目チェック機能を提供する。                                                                                                                                                          | -    | -                       |
| モーダル管理                        | HeadlessUIのモーダルダイアログの機能により、確認、完了ダイアログの表示、制御を実装支援する機能を提供する。                                                                                                                                                     | -    | -                       |
| 画面遷移制御                        | VueRouterにより、指定されたURLパス（または論理パス名）に対応する画面へ遷移する機能を提供する。                                                                                                                                                          | -    | -                       |
| HTTPクライアント<br/>（AJAX通信クライアント） | Axios、Axios Retryにより、クライアントからサーバへのAJAX通信(GET/POST/PUT/DELETE、ファイルアップロード、ダウンロード）、リトライ機能等を提供する。サーバのエラーレスポンスの形態に応じて集約例外ハンドリング機能と連動する例外をスローする。                                                                     | ○    | http-client             |
| 集約例外ハンドリング                    | クライアント側で、サーバからのエラーレスポンスを透過的に業務エラー、システムエラー等として扱い、例外の種類に応じて、専用エラー画面に遷移するといったシステム共通の集約的なエラー処理を実装支援する機能を提供する。                                                                                                      | ○    | errorhandler<br/>errors |
| ロギング                          | 開発中のデバッグを目的として、ブラウザの開発ツールのコンソールにログ出力するが、vite-plugin-remove-consoleで本番環境向けにはログが出ないように切り替え可能にする機能を提供する。                                                                                                          | -    | -                       |
| メッセージ管理                       | 画面やダイアログに表示するメッセージを一元管理する機能を提供する。                                                                                                                                                                              | ○    | messages                |
| セッション管理<br/>（グローバル状態管理）       | piniaにより、認証済のユーザ情報等、画面間で引継ぎアクセス可能な状態（グローバルな状態）を管理する機能を提供する。<br/>また、画面をリロードするなどしても状態を維持できるよう、Pinia Plugin PersistedstateでWebストレージに保存し管理できるようにする。                                                                | -    | -                       |
...
- [Preitter - Code formatter](https://marketplace.visualstudio.com/items?itemName=esbenp.prettier-vscode)
    ...
    *.md
    ...
    - vueファイルやtsファイルが、VSCodeでも自動保存時にPrettierでフォーマット、import文の整理がされるように、settings.jsonに以下を追記
